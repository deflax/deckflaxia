...
- GitHub Actions checks out `juce-framework/JUCE` at pinned `JUCE_REF=8.0.10` into `third_party/JUCE` before `DJAPP_REQUIRE_JUCE=ON` configure. That checkout is CI-only and must not be committed.
...
- Provide a licensed local checkout at `third_party/JUCE` and configure with `-DDJAPP_USE_VENDORED_JUCE=ON`.
...
```sh
./build-juce/DJApp --juce-shell-smoke-test --exit-after-init
./build-juce/DJApp --juce-ui-smoke-test --dump-components --exit-after-init
```
...
```sh
./build-juce/DJApp --playable-smoke-test --fixtures tests/fixtures/dj-workflow --exit-after-init
```

Production smoke:

```sh
./build-juce/DJApp --production-dj-workflow-smoke-test --fixtures tests/fixtures/dj-workflow --exit-after-init
```
...
./build-juce/DJApp --plugin-sandbox-smoke-test --kill-helper-after-ms 500 --fixtures tests/fixtures/plugins --exit-after-init
```

Performance smoke at the plan budget surface:

```sh
./build-juce/DJApp --performance-smoke-test --fixtures tests/fixtures/dj-workflow --sample-rate 48000 --buffer-size 512 --exit-after-init
```

Scope audit:

```sh
./build-juce/DJApp --scope-audit --forbid "Windows,recording,smart playlists,samplers,streaming,DVS,timecode,Rekordbox,Serato,cloud,accounts,marketplace,per-plugin sandbox,embedded plugin editor" --paths src tests docs cmake # deferred scope audit
```

## Plugin Sandbox Behavior

The sandbox MVP uses exactly five chain helpers: deck A, deck B, deck C, deck D, and master. `plugin-sandbox-helper-packaging-check` verifies that `DJAppPluginSandboxHelper` is built beside `DJApp` and that `DJAppPluginSandboxHelper --helper-smoke` emits its readiness line.
...
./build/DJApp --playable-smoke-test --fixtures tests/fixtures/dj-workflow --exit-after-init
./build/DJApp --production-dj-workflow-smoke-test --fixtures tests/fixtures/dj-workflow --exit-after-init
./build/DJApp --plugin-sandbox-smoke-test --kill-helper-after-ms 500 --fixtures tests/fixtures/plugins --exit-after-init
./build/DJApp --performance-smoke-test --fixtures tests/fixtures/dj-workflow --sample-rate 48000 --buffer-size 512 --exit-after-init
./build/DJApp --scope-audit --forbid "Windows,recording,smart playlists,samplers,streaming,DVS,timecode,Rekordbox,Serato,cloud,accounts,marketplace,per-plugin sandbox,embedded plugin editor" --paths src tests docs cmake # deferred scope audit
cmake --build build --target license-report
```
...
If `DJAPP_REQUIRE_JUCE=ON` configure fails, install or export JUCE and pass `-DCMAKE_PREFIX_PATH=/path/to/JUCE/install-or-build`, or add a licensed `third_party/JUCE` checkout and pass `-DDJAPP_USE_VENDORED_JUCE=ON`.
...
```sh
cmake -S . -B build-juce-runbook -DDJAPP_REQUIRE_JUCE=ON -DCMAKE_BUILD_TYPE=Debug -DCMAKE_EXPORT_COMPILE_COMMANDS=ON
cmake --build build-juce-runbook
./build-juce-runbook/DJApp --production-dj-workflow-smoke-test --fixtures tests/fixtures/dj-workflow --exit-after-init
```
